...
def quat_rotate_inverse(q, v):
    ...
    # 将 q_vec 和 v 重塑为合适的形状
    q_vec_reshaped = q_vec.reshape(shape[0], 3, 1)  # (batch_size, 3, 1)
    v_reshaped = v.reshape(shape[0], 1, 3)  # (batch_size, 1, 3)
    
    # 使用 np.matmul 进行矩阵乘法
    # 得到形状为 (batch_size, 3, 3) 的结果
    matmul_result = np.matmul(q_vec_reshaped, v_reshaped)  # (batch_size, 3, 3)
    
    # 提取对角线元素（每个 3x3 矩阵的对角线）并求和
    # 对角线元素的位置是 (i, i) for i in [0, 1, 2]
    c = np.diagonal(matmul_result, axis1=1, axis2=2)  # (batch_size, 3)
    
    # 计算最终结果
    c = c * q_vec  # 逐元素相乘，形状仍为 (batch_size, 3)
